  ...
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl) {
      if (ENV.googleOAuthClientId && ENV.googleOAuthClientSecret) {
        console.log(
          "[OAuth] OAUTH_SERVER_URL is not set; Google OAuth direct mode is enabled."
        );
      } else {
        console.error(
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    // Prefer Bearer token when available; fallback to session cookie.
    ...
    const sessionToken = bearerToken || sessionCookie || null;
    ...
    const createSessionBackedUser = (): User => {
      ...
      return {
        ...
        name: session.name,
        email: session.email,
        loginMethod: ENV.googleOAuthClientId ? "google" : null,
    ...
    if (isGoogleDirectMode) {
      return createSessionBackedUser();
    }
